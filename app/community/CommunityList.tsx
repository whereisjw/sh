...
const router = useRouter()
  ...
  return (
    ...
    {data?.getPosts?.map((post)=>(
      <div key={post.id} onClick={()=>router.push(`/community/${post.id}`)} className="flex cursor-pointer flex-col items-start ">
    <span className="flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100  text-gray-800">
      동네질문
    </span>
    <span className="mt-2 text-gray-700">
      <span className="text-teal-500 font-medium">Q.</span> {post?.question}
    </span>
